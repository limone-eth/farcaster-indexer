...
import {
  FarcasterUserTokenBalancesResults, FarcasterUserTokenBalancesVariables,
  FarcasterUserPOAPsAndNFTsResult,
  FarcasterUserPOAPsAndNFTsVariables
} from './interfaces';
import {FarcasterUserNFTsQuery, FarcasterUserPOAPsAndNFTsQuery, FarcasterUserPOAPsQuery} from './query.js';
 ...
 * @returns An array of results containing data from all pages.
 */
export const fetchFarcasterUserTokenBalances = async (
    variables: FarcasterUserTokenBalancesVariables
): Promise<FarcasterUserTokenBalancesResults[]> => {
  const allData: FarcasterUserTokenBalancesResults[] = [];

  // Fetch the first page of data
  let response: FetchQuery | null = await fetchQueryWithPagination(gqlToString(FarcasterUserNFTsQuery), variables);

  // Handle error for the first page
  if (response.error) {
    console.error(response.error);
    await delay(1000);
    return fetchFarcasterUserTokenBalances(variables);
  }
  ...
  while (shouldFetchNextPage) {
    ...
    // Log the number of API calls (optional)
    console.log(`API calls: ${numCalls++}`);
    //
    // hconsole.timeEnd('fetchFarcasterUserTokens');
  }
